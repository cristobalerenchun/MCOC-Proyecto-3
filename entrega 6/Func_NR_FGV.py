...
# DE UNA SECCIÓN TRAPEZOIDAL SIMÉTRICA GENERAL USANDO EL MÉTODO ESTÁNDAR
...
# S     >>> [m/m o pie/pie] Pendiente longitudinal del tramo
...
# FUNCIONES DE APOYO
from Func_geom import geom                          # Argumentos función = geom(b, z, y)
import math
...
def hidr(units, b, z, y, n, Q, S, T, A, R):
	...
	K  = C_0*A*abs(R)**(2./3.)/n   				 	# Conductividad hidráulica del modelo
...
def NR_FGV(units, b, z, y1, n, Q, S, dx, control):
	...
	while tol > 1e-10 and i <= 1000:
		...
		f   = a*(hd2['E'] - hd1['E']) + 0.5*(hd2['Sf'] + hd1['Sf'] - 2.*S)*dx
		df  = a*(math.cos(theta)**2. - alpha*hd2['Fr']**2.) - dx*hd2['Sf']*(5.*st2['T'] - 2.*st2['dP/dy']/st2['R'])/3.
		# Proceso de aproximaciones sucesivas:
		y_i = y2 - f/df							   # Formula de NR
		tol = abs(y_i - y2)						   # Tolerancia del paso
		y2  = y_i           					   # Mutación de y
		i   = i + 1
	return y2									   # Altura del agua buscada