...
ctrl   = 'DS'
	# ctrl >>> La altura y, conocida del problema se encuentra aguas abajo al final del tramo, correspondiente a  
	# la profundidad generada por la presa; por tanto, se asigna el valor 'DS' (Downstream).
y_ctrl =  5.
	# y_ctrl >>> Corresponde a la altura ym conocida del problema en la sección de control.
...
dx  = 1.   # [metros]
	...
	# uniforme se consideró pertinente emplearla en el análisis. Valores dx > 1.0 arrojaron diferencias no sub-metricas.
...
perfil = gfv(u, b, z, n, Q, S, dx, ctrl, y_ctrl, z_i)
# Datos adicionales solicitados:
yn = ynormal(u, n, S, z, b, Q)								# Altura normal del canal (para incorporar en el grafico)
yc = ycritical(u, z, b, Q)									# Altura critica del canal (para incorporar en el grafico)
...
walls    = max(array(perfil[1])) + 0.3 + array(perfil[2])	# Vector que representa la corona de los muros del canal
	...
	# se incorporan en el grafico únicamente con el objeto de dotar de contexto visual los resultados y mejorar la estética.
...
pyplot.plot(perfil[0], perfil[2], 'k', linewidth=4)
pyplot.plot(perfil[0], normal, 'k--')
pyplot.plot(perfil[0], critical, 'k:')
pyplot.plot(perfil[0], perfil[3], 'b', linewidth=2)
pyplot.plot(perfil[0], walls, 'k')
